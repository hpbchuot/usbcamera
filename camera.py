...
class Camera:
    ...
    def snapshot(self, flush=SNAPSHOT_FLUSH):
        """[chế độ paired] Mở (nếu chưa mở) -> XẢ vài khung cũ trong buffer -> đọc
        1 khung -> render vào self.cell. KHÔNG tự release (orchestrator đóng sau cả
        cặp). Trả True nếu cập nhật được ô. Lỗi -> cell=None (ô hiện 'mất kết nối'),
        nuốt exception để 1 cam hỏng không làm sập vòng chụp."""
        try:
